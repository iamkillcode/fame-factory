...
export default function TuneStreamPage() {
  const { gameState, isLoaded } = useGame();

  if (!isLoaded || !gameState.artist) {
    return <div className="flex h-full items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-primary" /> Loading TuneStream...</div>;
  }
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <SectionCard 
          ...
          icon={ListMusic} 
          className="bg-card/60 border-green-500/30"
        ...
        >
          {releasedSongs.length > 0 ? (
            ...
            <p className="text-center text-muted-foreground py-4">No tracks released on TuneStream yet.</p>
          )}
        </SectionCard>
        ...
        >
          {releasedAlbums.length > 0 ? (
            ...
            <p className="text-center text-muted-foreground py-4">No albums or EPs released on TuneStream yet.</p>
          )}
        </SectionCard>
      </div>
    </div>
